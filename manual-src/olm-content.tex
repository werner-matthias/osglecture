...
\ldeen{Standardmäßig bindet \option{patch-overlays=true} reine Ausdrücke aus
registrierten Modi in die nativen Modus- und Overlaybefehle von \cls{beamer}
und \cls{ltx-talk} ein. Eine Spezifikation wie \code{<longform>} wird dann als
portable Modusabfrage erkannt; eine numerische Spezifikation wie \code{<2->}
bleibt ein natives Overlay. Unbekannte oder gemischte Ausdrücke werden ebenfalls
an die Basisklasse weitergereicht. Wer diese Integration nicht möchte, setzt
\option{patch-overlays=false}; \cs{lecturemode} und
\cs{IfLectureModeTF} funktionieren weiterhin.}{By default,
\option{patch-overlays=true} integrates pure expressions made up of registered
modes into the native mode and overlay commands of \cls{beamer} and
\cls{ltx-talk}. A specification such as \code{<longform>} is then recognized
as a portable mode query, while a numeric specification such as \code{<2->}
remains a native overlay. Unknown or mixed expressions are also delegated to
the base class. Set \option{patch-overlays=false} to disable this integration;
\cs{lecturemode} and \cs{IfLectureModeTF} continue to work.}
