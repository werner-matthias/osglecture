...
\begin{description}
  \item[\cs{course}\oarg*{\ldeen{Modus}{mode}}\oarg{\ldeen{Kurzname}{short name}}\marg{\ldeen{Name}{name}}]
  ...
  \item[\cs{event}\oarg*{\ldeen{Modus}{mode}}\oarg{\ldeen{Kurzname}{short name}}\marg{\ldeen{Name}{name}}]
...
gesetzt?"'. Der Befehl \cs{lecturemode}\meta{modus}\marg{Inhalt} gibt
...
command \cs{lecturemode}\meta{mode}\marg{content} emits its content only
...
\begin{description}
  \item[\cs{lecturemode}\oarg*{mode expression}\marg{content}]
...
\ldeen{Für Erweiterungen des Modusgraphen stehen zusätzlich
\cs{DeclareLectureMode}, \cs{DeclareLectureModeParents},
\cs{DeclareLectureModeAlias}, \cs{SetLectureModes}, \cs{AddLectureModes} und
\cs{FinalizeLectureModes} zur Verfügung. Die Deklarationsphase endet mit der
Finalisierung; spätere Änderungen sind Fehler.}{Extensions of the mode graph
can additionally use \cs{DeclareLectureMode},
\cs{DeclareLectureModeParents}, \cs{DeclareLectureModeAlias},
\cs{SetLectureModes}, \cs{AddLectureModes}, and \cs{FinalizeLectureModes}.
Finalization ends the declaration phase; later changes are errors.}
...
\begin{description}
  ...
  \item[\cs{presitem}\oarg{relation, initial}\oarg*{overlay}\oarg{label}]
