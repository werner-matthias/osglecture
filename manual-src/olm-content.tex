...
\ldeen{Tragen Sie jede Projektsprache zuerst im Manifest ein
(\code{[targets.defaults].languages}, in fester Reihenfolge) und ordnen Sie sie
...
Konfiguration so aus}{First list every project language in the manifest
(\code{[targets.defaults].languages}, in a fixed order), then map it in
\File{Include/projectconfig.tex} to a name understood by \pkg{babel}.
...
\ldeen{Die auswählbaren Sprachen selbst werden hier nicht mehr wiederholt --
\osglecture\ übernimmt sie aus dem Manifest.}{The selectable languages
themselves are no longer repeated here -- \osglecture\ takes them from the
manifest.}
...
\begin{description}
  \item[\env{presitemize}\oarg{language, punctuation, whitespace}]
    ...
    \code{manual} und respektiert bei \code{auto} vorhandene Satzzeichen;
    \option{whitespace} akzeptiert \code{normalize} (Vorgabe) oder
    \code{preserve}}{produces a list in presentations and a grammatically joined
    paragraph in long forms. \option{language} overrides the detected language;
    \option{punctuation} accepts \code{auto} or \code{manual} and preserves
    existing punctuation in \code{auto} mode; \option{whitespace} accepts
    \code{normalize} (the default) or \code{preserve}}.
