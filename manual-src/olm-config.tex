...
\option{document\_metadata} mit \code{enabled} oder \code{disabled} ist eine
...
preset's profile defaults apply. \option{document\_metadata} with
\code{enabled} or \code{disabled} is a deliberate output choice and takes
effect only where the resolved profile leaves the metadata contract open
(\code{supported}); if it contradicts a profile with a forced contract,
...
\code{supported}-Profil, wenn \keyis{document\_metadata}{enabled} im Manifest
...
the manifest sets \keyis{document\_metadata}{enabled}. Symbols such as
...
\begin{description}
  \item[\cs{OsgLectureSetup}\marg{areas}] \ldeen{weist Basisklassenoptionen zu,
    adressiert nach Klassenname (\code{class/NAME}) oder aktivem Modus
    (\code{class/<MODE>}), und mit \code{mode/NAME} osglecture-eigene
    modusabhängige Einstellungen}{assigns base-class options addressed by class
    name (\code{class/NAME}) or active mode (\code{class/<MODE>}), and
    osglecture mode-dependent settings with \code{mode/NAME}}.
