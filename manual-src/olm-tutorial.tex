...
\key*{presentation\_profile}{ltx-talk} in \code{[targets.defaults]} des
...
\key*l{presentation\_profile}{ltx-talk} would go into the manifest's
...
und \code{british}. Der Schlüssel \option{languages} reicht dieses
\option{map} an \pkg{langselect} weiter und lädt \pkg{babel}. Welche Sprachen
das Projekt überhaupt anbietet, steht bereits im Manifest -- \osglecture\
reicht die Liste von dort an \pkg{langselect}.}{The remaining language
configuration contains two different kinds of names. \OLLM\ uses the short,
portable identifiers \code{de} and \code{en}. The popular language package
\pkg{babel}, by contrast, needs the more specific names \code{ngerman} and
\code{british}. The \option{languages} option passes this \option{map} on to
\pkg{langselect} and loads \pkg{babel}. Which languages the project offers at
all is already in the manifest -- \osglecture\ hands that list from there to
\pkg{langselect}.}
