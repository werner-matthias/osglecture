...
einerseits an \cls{osglecture} (genauer gesagt: an das zu \osglecture\ gehörende \pkg{langselect})
weiter und lädt auch das Paket \pkg{babel}; Nora muss
...
\code{ngerman} and \code{british}. The \option{languages} option passes this configuration
on to \cls{osglecture} (more precisely, to \pkg{langselect}, which is associated with \osglecture)
and also loads the \pkg{babel} package; Nora doesn’t have to
rethink this division of labor for every paragraph; the configuration
