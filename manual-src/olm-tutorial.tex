...
\ldeen{Die Klassen \cls{beamer} und \cls{scrbook} sind bereits die Vorgaben des
Bundle-Presets; wollte Nora stattdessen \cls{ltx-talk}, käme
\keyis{presentation\_profile}{ltx-talk} in \code{[targets.defaults]} des
Manifests, nicht hierher. Denn die Klassenwahl muss feststehen, bevor die
Klasse überhaupt startet.}{The classes \cls{beamer} and \cls{scrbook} are
already the bundle preset's defaults; if Nora wanted \cls{ltx-talk} instead,
\keyis{presentation\_profile}{ltx-talk} would go into the manifest's
\code{[targets.defaults]}, not here -- the class choice has to be fixed before
the class even starts.}

\ldeen{Die verbleibende Sprachkonfiguration enthält zwei verschiedene Arten von
Namen. \OLLM\ arbeitet mit den kurzen, portablen Kennungen \code{de} und
\code{en}, wie sie in der Norm ISO 639-1 definiert sind. Das verbreitete
Sprachpaket \pkg{babel} benötigt dagegen die konkreteren Namen \code{ngerman}
und \code{british}. Der Schlüssel \option{languages} reicht dieses Mapping an
\pkg{langselect} weiter und lädt \pkg{babel}. Welche Sprachen das Projekt
überhaupt anbietet, steht bereits im Manifest -- \osglecture\ reicht die Liste
von dort an \pkg{langselect}.}{The remaining language configuration contains
two different kinds of names. \OLLM\ uses the short, portable identifiers
\code{de} and \code{en}. The popular language package \pkg{babel}, by
contrast, needs the more specific names \code{ngerman} and \code{british}. The
\option{languages} key passes this mapping on to \pkg{langselect} and loads
\pkg{babel}. Which languages the project offers at all is already in the
manifest -- \osglecture\ hands that list from there to \pkg{langselect}.}
...
\begin{document}
\lecture[Memory]{\ldeen{Virtueller Speicher}{Virtual Memory}}{lec:memory}
...
\code{lec:memory}, ist das Label der Unterrichtseinheit
...
\code{lec:memory}, is the label of the unit. It constitutes the unit's stable identity.
...
\ldeen{Die \env{frame}-Umgebung kennt Nora schon aus ihrer früheren Arbeit mit \cls{beamer}.
...
\env{frame} environment from her earlier work with \cls{beamer}. She often
...
\begin{infobox}{\ldeen{Hinweis}{Hint}}\sffamily
