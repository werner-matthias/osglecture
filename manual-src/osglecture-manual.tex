...
\ldeen{Dass auch gewöhnlich Textauszeichnungen unterschiedlich gestaltet und damit polymorph sein können, ist in \LaTeX\ ein alter Hut. \osglecture\ unterstützt dies aber kompfortable durch spezifische Profile.}{It is well known in \LaTeX\ that text markup can vary and thus be polymorphic. However, \osglecture\ provides convenient support for this through specific profiles.}
...
\ldeen{Warum man solche Polymorphie überhaupt \emph{braucht}, wird im folgendem Kurztutorial demonstriert.
}{The following short tutorial demonstrates why this kind of polymorphism \emph{is needed at all }in the first place.
}
...
Die erste Unterrichtseinheit soll über Speicher gehen. Nora will sofort Speichervirualisierung behandeln. 
...
dass viele Gaststudenten teilnehmen werden, die besser in Englisch als in Deutsch sind.
Daher will sie die Folien jeweils in beiden Sprachen haben.
...
die Dinge, die Nora für ihren ersten kleinen Kurs tatsächlich benötigt.\footnote{%
 ...
 Am Ende werden aus einer Lehreinheit drei sichtbar verschiedene PDFs entstehen.}{The
...
\texttt{slides-en.tex} und beginnt mit der Übersetzung. Für das Skript fügt sie
später noch \texttt{script-de.tex} hinzu. Das funktioniert. 
...
She creates \texttt{slides-de.tex}, copies the file to
\texttt{slides-en.tex}, and begins the translation. For the script, she
later adds \texttt{script-de.tex}. That works.
...
Die Kombination "`Unterrichtseinheit \texttt{memory}, Target \texttt{slides}, Sprache
...
For example, the combination "`Lesson \texttt{memory}, Target \texttt{slides}, Language
\texttt{en}"' generates the English slide instance of this lesson.
...
\texttt{ollm slides}. However, the tool cannot yet know which languages the course offers
...
Tippfehler korrigiert hat, ist das Manifest gültig. 
Auch falls ihre Buildumgebung selbst Probleme bereitet, 
liefert \texttt{ollm doctor} die passendere Diagnose.}{
On her first attempt, she had written \texttt{language} instead of
\texttt{languages}. OLLM rejects the unknown key.
...
\texttt{ollm doctor} provides a more accurate diagnosis.
}
...
KOMA-Scripts \texttt{scrbook} gesetzt werden. 
Einen Moment lang überlegt sie, ob sie nicht doch lieber die modernere Präsentationsklasse 
\texttt{ltx-talk} nehmen soll, um damit ihre Dokumente barrierefrei anbieten zu können,
aber sie schreckt davor zurück, da \texttt{ltx-talk} noch nicht als stabil gilt.
Außerdem möchte sie die Folien im offiziellen Layout ihrer Universität anbieten,
und das gibt es für \texttt{ltx-talk} noch nicht.
...
\texttt{ltx-talk} so she can make her documents accessible,
but she hesitates because \texttt{ltx-talk} is not yet considered stable.
In addition, she wants to present the slides in her university’s official layout,
and that isn’t available for \texttt{ltx-talk} yet.
...
\texttt{british}. Der Schlüssel \texttt{languages} reicht diese Konfiguration
an \texttt{langselect} weiter und lädt das Paket; Nora muss
...
\texttt{ngerman} and \texttt{british}. The \texttt{languages} key passes this
configuration to \texttt{langselect} and loads the package. Nora need not
...
\texttt{memory}, ist das Label der Unterrichtseinheit 
Dieses bildet ihre stabile Identität. Nora darf den sichtbaren
...
\texttt{memory}, is the lable of the unit. It constitutes the unit's stable identity. 
...
verwendet sie die \texttt{presitemize}-Umgebung}{Nora adds two more statements. An ordinary
...
\ldeen{Sie hätte auch \texttt{<script>} schreiben können. Jedoch sagt \texttt{<longform>}
...
\ldeen{Die optionale Adresse vor dem Label bezeichnet eine andere logische
Lehreinheit derselben Serie. Standardmäßig verwendet der Verweis dabei
...
\ldeen{Mit den Schlüsseln \texttt{type} und \texttt{lang} lässt sich diese
Projektion ausdrücklich überschreiben. Das ist ein dokumenttypübergreifender
...
\include{osglecture-manual-project-configuration}
\include{osglecture-manual-ollm}
\include{osglecture-manual-series-standalone}
\end{document}
