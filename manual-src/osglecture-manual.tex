...
Die folgenden Beispiele sind keine nachgebauten Ausgaben, sondern Ausschnitte aus
der mitgelieferten Beispielserie \texttt{examples/series-modern}.}{It is a
...
name from this concept. The following examples are not reconstructed output;
they are excerpts from the included \texttt{examples/series-modern} example
series.}

\ldeen{Die folgende Aussagenfolge stammt unverändert aus der Unit
\texttt{010-introduction/main.tex}. Sie ist die \emph{eine Quelle} für alle
darunter gezeigten Dokumente. \cs{lende} hält die beiden Sprachfassungen
beieinander. \cs{presitemize}, \cs{anditem} und \cs{thereforeitem} zeichnen
nicht das Aussehen, sondern den Zusammenhang der Aussagen aus; \cs{finite}
markiert das finite Verb für die Langform.}{The following sequence of
statements is taken unchanged from the unit
\texttt{010-introduction/main.tex}. It is the \emph{one source} for all
documents shown below. \cs{lende} keeps the two language versions together.
\cs{presitemize}, \cs{anditem}, and \cs{thereforeitem} mark the relations
between statements rather than their appearance; \cs{finite} marks the finite
verb needed by the long-form rendering.}

\ldeen{Die zugehörigen kleinen Treiberdateien liegen ebenfalls in
\texttt{manual-src}; \texttt{l3build doc} erzeugt daraus vor dem Handbuch die
hier eingebetteten PDFs. Änderungen an den beiden gemeinsamen Quelldateien
werden dadurch unmittelbar in allen Varianten sichtbar.}{The corresponding
small driver files also live in \texttt{manual-src}; before typesetting the
manual, \texttt{l3build doc} generates the embedded PDFs from them. Changes to
the two shared source files therefore become visible in every variant
immediately.}
...
\ldeen{Zunächst bleibt der Dokumenttyp \texttt{slides} fest; nur die Sprache
wechselt. Gliederung, Aussagenfolge und Hervorhebung von \emph{talk} und
\emph{script} sind in beiden Ergebnissen dieselben.}{First the document type
\texttt{slides} remains fixed and only the language changes. Structure,
statement sequence, and the emphasis of \emph{talk} and \emph{script} are the
same in both results.}

\begin{figure}[htbp]
  \centering
  \begin{minipage}[t]{.48\linewidth}
    \centering\textbf{\texttt{slides}, \texttt{en}}\par\smallskip
    \fbox{\includegraphics[page=1,width=\linewidth]{polymorphy-slides-en.pdf}}
  \end{minipage}\hfill
  \begin{minipage}[t]{.48\linewidth}
    \centering\textbf{\texttt{slides}, \texttt{de}}\par\smallskip
   \fbox{\includegraphics[page=1,width=\linewidth]{polymorphy-slides-de.pdf}}
  \end{minipage}
  \caption{\ldeen{Gleicher Doctype, unterschiedliche Sprachwahl.}%
    {The same document type with different language selections.}}
\end{figure}

\paragraph{\ldeen{Eine Quelle, zwei Dokumenttypen und Formulierungen}%
  {One source, two document types and phrasings}.}
\ldeen{Nun bleibt die Sprache Deutsch fest; nur der Dokumenttyp wechselt. Das
Präsentationsprofil setzt die Aussagen als Liste. Der Langformadapter verwendet
die ausgezeichneten Beziehungen und finiten Verben, um daraus einen Satz zu
bilden. Auch die Textauszeichnung \cs{emph} bleibt semantisch dieselbe und wird
vom jeweiligen Profil passend umgesetzt.}{Now the language remains German and
only the document type changes. The presentation profile renders the statements
as a list. The long-form adapter uses the marked relations and finite verbs to
form a sentence. The semantic text markup \cs{emph} also remains the same and
is rendered appropriately by each profile.}
...
\begin{figure}[htbp]
  ...
  \end{minipage}\hfill
  \begin{minipage}[t]{.48\linewidth}
    \centering\textbf{\texttt{script}, \texttt{de}}\par\smallskip
    \fbox{\includegraphics[page=1,width=\linewidth,
      trim=38mm 198mm 38mm 15mm,clip]{polymorphy-script-de.pdf}}
  \end{minipage}
  \caption{\ldeen{Gleiche Sprache, unterschiedliche Dokumentgestalt und
    Formulierung.}{The same language with different document form and phrasing.}}
\end{figure}

\paragraph{\ldeen{Eine Quelle, verschiedene Darstellungen mit \texttt{osgstyler}}%
  {One source, different rendering with \texttt{osgstyler}}.}
\ldeen{Auch gewöhnliche Autorenauszeichnungen können polymorph sein. Der
folgende Text enthält nur die semantischen Befehle \cs{emph} und \cs{textbf}.
Die Bildschirm- und Druckprofile binden sie an unterschiedliche
\texttt{osgstyler}-Ressourcen, ohne den Inhalt zu duplizieren.}{Ordinary author
markup can be polymorphic as well. The following text contains only the
semantic commands \cs{emph} and \cs{textbf}. The screen and print profiles bind
them to different \texttt{osgstyler} resources without duplicating the content.}
...
\begin{figure}[htbp]
  \centering
  \begin{minipage}[t]{.48\linewidth}
    \centering\textbf{\ldeen{Bildschirmprofil}{Screen profile}}\par\smallskip
    \includegraphics[page=1,width=\linewidth,
      trim=15mm 150mm 15mm 15mm,clip]{styler-screen.pdf}
  \end{minipage}\hfill
  \begin{minipage}[t]{.48\linewidth}
    \centering\textbf{\ldeen{Druckprofil}{Print profile}}\par\smallskip
    \includegraphics[page=1,width=\linewidth,
      trim=15mm 150mm 15mm 15mm,clip]{styler-print.pdf}
  \end{minipage}
  \caption{\ldeen{Gleiche semantische Auszeichnung, unterschiedliche
    \texttt{osgstyler}-Ressourcen.}{The same semantic markup with different
    \texttt{osgstyler} resources.}}
\end{figure}
