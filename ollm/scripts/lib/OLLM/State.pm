...
sub write_registry {
  ...
  for my $result (@results) {
    ...
    my ($physical, $unit) = @{$result}{qw(physical_unit unit_id)};
    die "physical unit '$physical' has conflicting logical identities\n"
      if exists($physical{$physical}) && $physical{$physical} ne $unit;
    die "logical unit '$unit' belongs to more than one physical unit\n"
      if exists($logical{$unit}) && $logical{$unit} ne $physical;
    $physical{$physical} = $unit;
    $logical{$unit} = $physical;
  }
  for my $physical (sort keys %physical) {
    push @lines,
      '\\OsgLectureSeriesUnit{physical={' . _tex_value($physical)
      . '},unit={' . _tex_value($physical{$physical}) . '}}';
  }
  for my $result (@results) {
    next if ($result->{unit_role} // '') eq 'i';
...
sub promote {
  ...
  eval {
    ...
    my $record = {
      ...
      physical_number  => $spec->{physical_number},
      ...
      source           => $spec->{source},
      shell_escape     => $spec->{shell_escape},
      bundle_preset    => $spec->{bundle_preset},
      structure_signature => $spec->{structure_signature},
      project_config_signature => $spec->{project_config_signature},
      document_metadata_signature => $spec->{document_metadata}
        ? $spec->{document_metadata}{signature} : undef,
